...
router.post('/register', (req, res) => {
    ...
    //     let messages = [];
    //     messages.push({text, color: 'red darken-4'});
    ...
    if (!username || !password || username === '' || password === '') {
        ...
        handleError(req, res, 'Please submit a username and a password.', 'register');
        return;
    }
...
router.post('/login', (req, res, next) => {
    ...
    MongoClient.connect(config.database, (err, db) => {
        ...
        db.collection('users').find({username}).toArray((err, docs) => {
            ...
            let hash = userFound ? docs.pop().password : '$2a$16$G6BB8k/knhnof8jD7OaLQuDTJYhCJbk0VUVO1CbVkgcmbbhQn.boK';
